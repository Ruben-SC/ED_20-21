...
public:
	...
	void colarse(Queue<int> amigos, int pos) {
		...
		if (encontrado) {
			...
			if (actual == _ult) {
				...
				amigos._ult->_sig = actual->_sig;
			}

			actual->_sig = amigos._prim;

			_numElems += amigos._numElems;
		...
		// Coste lineal O(n) siendo 'n' el tamaño de la 'cola', ya que el modificar las asignaciones de los atributos de los nodos es constante
	}
