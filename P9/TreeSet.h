...
public:
	...
	// en el peor de los casos sería coste lineal O(n) al recorrerlo entero, cuando ambos extremos entran en las cotas, 
	// en otros casos sería coste logarítmico O(log n) dado que al ser ordenado puede ignorarse partes que no cumplan con el objetivo
	void count_interval_aux(const T& lower, const T& upper, int& cant, Nodo* arbol) const {
		
		if (arbol != NULL) {
			if (arbol->_elem >= lower && arbol->_elem <= upper)
				cant++;

			if (arbol->_iz != NULL && arbol->_iz->_elem >= lower || arbol->_elem > lower)
				count_interval_aux(lower, upper, cant, arbol->_iz);
			if (arbol->_dr != NULL && arbol->_dr->_elem <= upper || arbol->_elem < upper)
				count_interval_aux(lower, upper, cant, arbol->_dr);
		}
	}
	...
	int count_interval(const T& lower, const T& upper) const {
		...
		count_interval_aux(lower, upper, cant, _ra);
